...
export async function GET(req: Request) {
  ...
  if (type === 'transactions') {
    ...
    const rows = (data || []).map(inv => ({
      ...
      customer_name: (Array.isArray(inv.customers) ? inv.customers[0]?.name : (inv.customers as { name: string; email: string } | null)?.name) || '',
      customer_email: (Array.isArray(inv.customers) ? inv.customers[0]?.email : (inv.customers as { name: string; email: string } | null)?.email) || '',
